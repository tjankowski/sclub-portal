  ...
  protected
    ...
    def access_denied
      respond_to do |format|
        format.html do
          store_location
          flash[:error] = "Musisz się zalogować, aby skorzystać z tej funkcji."
    ...
    def permission_denied
      respond_to do |format|
        format.html do
          ...
          domain_name = "http://ksmotobi.pl"
          http_referer = session[:refer_to]
          if http_referer.nil?
            store_referer
            http_referer = ( session[:refer_to] || domain_name )
          end
          flash[:error] = "Nie masz uprawnień do zakończenia tej akcji."
        ...
        format.xml do
          ...
          render :text => "Nie masz uprawnień do zakończenia tej akcji.", :status => '401 Unauthorized'
        end
      end
    end
